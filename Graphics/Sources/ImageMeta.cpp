					...
					const UniChar inSep) 
{
	...
	uLONG degreeTotal = 0; //in milliseconds
	if (vCoords.size() >= 1)
		degreeTotal = floor(fabs(vCoords[0].GetReal())*3600*1000+0.5);
	if (vCoords.size() >= 2)
		degreeTotal += floor(fabs(vCoords[1].GetReal())*60*1000+0.5);
	if (vCoords.size() >= 3)
		degreeTotal += floor(fabs(vCoords[2].GetReal()*1000+0.5));

	outDegree = degreeTotal/(3600*1000);
	degreeTotal = degreeTotal%(3600*1000);
	outMinute = degreeTotal/(60*1000);
	outSecond = degreeTotal%(60*1000);
	outSecond = outSecond/1000;
					...
					const UniChar inSep) 
{
	//normalize degree, minute, and second
	uLONG degreeTotal = 0; //in milliseconds
	degreeTotal = floor(fabs(inDegree)*3600*1000+0.5);
	degreeTotal += floor(fabs(inMinute)*60*1000+0.5);
	degreeTotal += floor(fabs(inSecond)*1000+0.5);

	uLONG degree, minute;
	Real second;
	degree = degreeTotal/(3600*1000);
	degreeTotal = degreeTotal%(3600*1000);
	minute = degreeTotal/(60*1000);
	second = degreeTotal%(60*1000);
	second = second/1000;
	if (second <= 0.00001)
		second = 0;
