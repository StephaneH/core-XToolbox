...
XWinFont::XWinFont (VFont *inFont, const VString& inFontFamilyName, const VFontFace& inFace, GReal inSize):fFont( inFont)
{
	...
	fIsTrueType = _IsFontTrueType(hDC, fFontRef); //return true for all scalable fonts
	if (fIsTrueType) 
	{
		//if font is scalable but not supported by D2D or GDIPlus we should fallback also to GDI (for consistency) and so set flag to false 
#if ENABLE_D2D
		if (fD2DShouldFallbackGDI)
			fIsTrueType = false;
		else 
#endif
			if (fGDIPlusShouldFallbackGDI)
				fIsTrueType = false;
	}
...
bool XWinFont::_GetName(IDWriteLocalizedStrings *inLocalStrings, VString& outName, bool inLocalized) const
{
	UINT32 index = 0;
	BOOL exists = FALSE;
	VString localName = "en-US"; //default is neutral
	...
	if (inLocalized)
	{
		...
		localName = gLocalName;
	} 

	inLocalStrings->FindLocaleName( localName.GetCPointer(), &index, &exists);
	if (!exists && inLocalized)
	{
		localName = "en-US"; //backfall to neutral
		inLocalStrings->FindLocaleName( localName.GetCPointer(), &index, &exists);
	}
	if (exists)
	{
		...
		UINT32 length = 0;
		inLocalStrings->GetStringLength( index, &length);
		if (length)
		{
			VPtr buffer = VMemory::NewPtr( (length+1)*sizeof(UniChar), 'name');
			inLocalStrings->GetString( index, (WCHAR *)buffer, length+1);
			outName.FromUniCString( (const UniChar *)buffer);
...
void XWinFont::GetName (eFontNameKind inFontNameKind, VString& outName, bool inLocalized) const
{
	if (inFontNameKind == kFONTNAMEKIND_DESIGN)
	{
		outName = fFont->fDesignName;
		return;
	}
	
	//on Windows, we can only get font family, not postcript or full name

#if ENABLE_D2D
	if (fDWriteFont && !fD2DShouldFallbackGDI)
	{
		if (inFontNameKind == kFONTNAMEKIND_FAMILY)
		{
			CComPtr<IDWriteFontFamily> fontFamily;
			if (SUCCEEDED(fDWriteFont->GetFontFamily(&fontFamily)))
			{
				CComPtr<IDWriteLocalizedStrings> localNames;
				fontFamily->GetFamilyNames(&localNames);

				bool done = _GetName( localNames, outName, inLocalized);
				if (done)
					return;
			}
		}
#if TRUE				
		else if (inFontNameKind == kFONTNAMEKIND_POSTSCRIPT)
		{
			CComPtr<IDWriteLocalizedStrings> localNames;
			BOOL exists = FALSE;
			if (SUCCEEDED(fDWriteFont->GetInformationalStrings( DWRITE_INFORMATIONAL_STRING_POSTSCRIPT_NAME, &localNames, &exists)) && exists)
			{
				bool done = _GetName( localNames, outName, inLocalized);
				if (done)
					return;
		...
		else if (inFontNameKind == kFONTNAMEKIND_FULL)
		{
			CComPtr<IDWriteLocalizedStrings> localNames;
			BOOL exists = FALSE;
			if (SUCCEEDED(fDWriteFont->GetInformationalStrings( DWRITE_INFORMATIONAL_STRING_FULL_NAME, &localNames, &exists)) && exists)
			{
				bool done = _GetName( localNames, outName, inLocalized);
				if (done)
					return;
...
#endif
	if (fGDIPlusFont && !fGDIPlusShouldFallbackGDI)
	{
		if (inFontNameKind == kFONTNAMEKIND_FAMILY)
		{
			LANGID langID = inLocalized ? LANGIDFROMLCID(GetUserDefaultLCID()) : LANG_NEUTRAL;

			Gdiplus::FontFamily *family = new Gdiplus::FontFamily();
			Gdiplus::Status status = fGDIPlusFont->GetFamily(family);
			WCHAR familyName[LF_FACESIZE];
			status = family->GetFamilyName(familyName, langID);
			if (status != Gdiplus::Ok && langID != LANG_NEUTRAL)
				status = family->GetFamilyName(familyName, LANG_NEUTRAL); //should always succeed
			xbox_assert(status == Gdiplus::Ok);
			xbox_assert(sizeof(UniChar) == sizeof(WCHAR));
			outName.FromUniCString((const UniChar *)familyName);
			delete family;
			return;
		}
	}
	if (inFontNameKind == kFONTNAMEKIND_FAMILY)
	{
		GetRealFamilyName( outName);
		return;
	}
	outName = fFont->fDesignName;
}

Boolean XWinFont::IsTrueTypeFont() const
{
...
/** font may be substituted: this method will return the real family name and not the one passed to RetainFont at font creation */
void XWinFont::GetRealFamilyName(VString& outName, const GraphicContextType _inGCType, const TextRenderingMode inTRM) const
{
	GraphicContextType type = _inGCType;
	if (type == eDefaultGraphicContext)
	{
		//default depends on current system capability (we force default to be a advanced gc in order to get correct family name for truetype fonts)
#if ENABLE_D2D
		type = eWinD2DSoftwareGraphicContext;
#else
		type = eWinGDIPlusGraphicContext;
#endif	
	}

#if ENABLE_D2D
	if (type == eWinD2DSoftwareGraphicContext || type == eWinD2DHardwareGraphicContext)
	{
		if (IsTrueTypeFont() || (inTRM & TRM_LEGACY_OFF))
		{ 
			VTaskLock lock(&VWinD2DGraphicContext::GetMutexDWriteFactory());
			if (GetDWriteTextFormat() != NULL)
			{
				UINT32 nameLength = GetDWriteTextFormat()->GetFontFamilyNameLength();
				if (nameLength + 1 <= 80)
				{
					WCHAR familyName[80];
					HRESULT hr = GetDWriteTextFormat()->GetFontFamilyName(familyName, nameLength + 1);
					xbox_assert(sizeof(UniChar) == sizeof(WCHAR));
					outName.FromUniCString( (const UniChar *)familyName);
					return;
				}
...
#endif
		if (type == eWinGDIPlusGraphicContext)
		{
			if (IsTrueTypeFont() || (inTRM & TRM_LEGACY_OFF))
			{
				Gdiplus::FontFamily *family = new Gdiplus::FontFamily();
				Gdiplus::Status status = GetGDIPlusFont()->GetFamily(family);
				WCHAR familyName[LF_FACESIZE];
				status = family->GetFamilyName(familyName, LANGIDFROMLCID(GetUserDefaultLCID()));
				if (status != Gdiplus::Ok)
					status = family->GetFamilyName(familyName); //should always succeed
				xbox_assert(sizeof(UniChar) == sizeof(WCHAR));
				outName.FromUniCString((const UniChar *)familyName);
				delete family;
	...
	HFONT hFont = GetFontRef();
	if (!hFont)
		outName = fFont->GetName();
	else
	{
		...
		if (!GetObjectW(hFont, sizeof(LOGFONTW), &lf))
			return;
		WCHAR *familyName = &(lf.lfFaceName[0]);
		xbox_assert(sizeof(UniChar) == sizeof(WCHAR));
		outName.FromUniCString((const UniChar *)familyName);
	}
}
...
void XWinFont::_GetStyleFromFontName(const VString &inFontFullName, VString &outFontFamilyName, int& outStyle, int& outWeight, int& outStretch) const
{
	...
	//JQ 22/05/2014: fixed ACI0086730: replace '-' characters with white-space if it is postcript name
	outFontFamilyName.ExchangeAll('-',' ');
	VIndex pos;
...
FontRef XWinFont::_CreateFontRef (const VString& _inFontFamilyName, const VFontFace& inFace, GReal inSize) 
{
	...
	if (fGDIPlusFont)
		delete fGDIPlusFont;
	bool isGDIPlusFontAvailable = true;
...
#endif
	isGDIPlusFontAvailable = fGDIPlusFont->IsAvailable() != FALSE;
	...
	if (fontFamily->IsAvailable())
	{
		...
		fGDIPlusShouldFallbackGDI = !(inFontFamilyName.BeginsWith(sFontFamilyDefault) && (inFontFamilyName.GetLength() == sFontFamilyDefault.GetLength() || inFontFamilyName[sFontFamilyDefault.GetLength()] == ' '));

		isGDIPlusFontAvailable = !fGDIPlusShouldFallbackGDI;
	}
	else
		isGDIPlusFontAvailable = false;

	delete fontFamily;

	if (!isGDIPlusFontAvailable)
	{
		//try to substitute font
		fGDIPlusShouldFallbackGDI = true;

		//JQ 22/05/2014: fixed ACI0086730: try to convert from postcript name to family name
		VString fontname = inFontFamilyName;
		fontname.ExchangeAll('-',' ');
		if (fontname != inFontFamilyName && VFont::FontGDIExists(fontname))
		{
			fGDIPlusShouldFallbackGDI = false;

			if (fGDIPlusFont)
				delete fGDIPlusFont;
			fGDIPlusFont = new Gdiplus::Font((const WCHAR *)fontname.GetCPointer(), inSizePx, gdiplusface, Gdiplus::UnitPixel);
			...
			if (fontFamily->IsAvailable())
			{
				...
				fGDIPlusShouldFallbackGDI = !(fontname.BeginsWith(sFontFamilyDefault) && (fontname.GetLength() == sFontFamilyDefault.GetLength() || fontname[sFontFamilyDefault.GetLength()] == ' '));
			}
			else
				fGDIPlusShouldFallbackGDI = true;
			delete fontFamily;
		...
		{
			fontname.SetEmpty();
			VFont::GetFontFamilySubstitute(inFontFamilyName, fontname, eWinGDIPlusGraphicContext, TRM_LEGACY_OFF); //note: for substitution, table is same for GDIPlus & D2D
			if (fontname != inFontFamilyName)
			{
				if (fGDIPlusFont)
					delete fGDIPlusFont;
				fGDIPlusFont = new Gdiplus::Font((const WCHAR *)fontname.GetCPointer(), inSizePx, gdiplusface, Gdiplus::UnitPixel);
	...
	//fixed ACI0086730: no matter if postscript or family name
	xbox_assert(sizeof(UniChar) == sizeof(WCHAR));
	VString name1( (const UniChar *)logfont.lfFaceName);
	VString name2( (const UniChar *)logfont2.lfFaceName);
	name1.ExchangeAll('-',' ');
	name2.ExchangeAll('-',' ');
	if (name1 == name2)
...
#if ENABLE_CUSTOM_FONTS_SUBSTITUTES
	if (hfont && VFont::HasFontFamilySubstitutes())
	{
		bool exists = VFont::FontGDIExists(fontFamilyNameGDI);
		if (!exists)
		{
			//try to substitute
			VString fontname;
			VFont::GetFontFamilySubstitute(fontFamilyNameGDI, fontname, eWinGDIGraphicContext);
			if (fontFamilyNameGDI != fontname && fontname.GetLength() < LF_FACESIZE)
			{
				xbox_assert(sizeof(WCHAR) == sizeof(UniChar));
				wcscpy(&(logfont.lfFaceName[0]), (const WCHAR *)fontname.GetCPointer());
