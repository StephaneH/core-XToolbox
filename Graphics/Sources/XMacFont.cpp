...
XMacFont::XMacFont( VFont* inFont, const VString& _inFontFamilyName, const VFontFace& inFace, CGFloat inSize):fFont( inFont)
{
    VString inFontFamilyName = _inFontFamilyName;
    VFont::UnLocalizeFontName(inFontFamilyName); //CTFontCreateWithName accepts only neutral font name...
	...
	if (inFace == 0 && inSize == 12.0)
	{
		...
		if (fCTFontRef)
		{
			CFStringRef name = CTFontCopyFamilyName(fCTFontRef);
			VString xname;
			xname.MAC_FromCFString(name);
			CFRelease(name);

			if (!(inFontFamilyName.BeginsWith(xname) && (inFontFamilyName.GetLength() == xname.GetLength() || inFontFamilyName[xname.GetLength()] == ' ')))
			{
                //try to substitute
                
                VString fontname;
                VFont::GetFontFamilySubstitute(inFontFamilyName, fontname);
                if (fontname != inFontFamilyName)
                {
                    CFRelease(fCTFontRef);
                    
                    CFStringRef familyName = fontname.MAC_RetainCFStringCopy();
                    fCTFontRef = CTFontCreateWithName(familyName, (CGFloat) 12.0, NULL);
                    CFRelease(familyName);
                    if (!fCTFontRef)
                    {
...
/* return font name according to passed kind name (localized or not) */
void XMacFont::GetName (eFontNameKind inFontNameKind, VString& outName, bool inLocalized) const
{
	...
	switch (inFontNameKind)
	{
		case kFONTNAMEKIND_FAMILY:
			namekey = kCTFontFamilyNameKey;
			break;
		case kFONTNAMEKIND_POSTSCRIPT: 
			namekey = kCTFontPostScriptNameKey;
			break;
		case kFONTNAMEKIND_FULL:
			namekey = kCTFontFullNameKey;
			break;
		default:
			xbox_assert(false);
			break;
	}
	name = inLocalized ? CTFontCopyLocalizedName( ctfont, namekey, NULL) : CTFontCopyName( ctfont, namekey);
	if (name)
	{
		outName.MAC_FromCFString(name);
		CFRelease(name);
	}
	if (!name)
	{
		...
		{
			//try unlocalized
			GetName( inFontNameKind, outName, false);
			return;
		}
		if (inFontNameKind != kFONTNAMEKIND_FAMILY)
		{
			//fallback to font family
			GetName( kFONTNAMEKIND_FAMILY, outName, inLocalized);
			return;
		}
		//fallback to design name
		outName = fFont->fDesignName;
	}
}
...
void XMacFont::GetRealFamilyName(VString& outName, const GraphicContextType /*inGCType*/, const TextRenderingMode /*inTRM*/) const
{
	CTFontRef ctfont = GetFontRef();
