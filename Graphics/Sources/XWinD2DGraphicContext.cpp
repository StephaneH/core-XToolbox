...
VLayerClipElem::VLayerClipElem( const VAffineTransform& inCTM, const VRect& inBounds, const VRect& inPrevClipBounds, const VRegion *inPrevClipRegion,
								...
								bool inHasClipOverride, bool inOwnOffScreen, VGraphicFilterProcess *inFilter)
{
	fCTM = inCTM;
...
VLayerClipElem::VLayerClipElem( const VLayerClipElem& inClip)
{
	...
	fFilter = NULL;
	CopyRefCountable(&fFilter, inClip.fFilter);
}
...
bool VWinD2DGraphicContext::_BeginLayer(const VRect& _inBounds, GReal inAlpha, VGraphicFilterProcess *inFilterProcess, VImageOffScreenRef inBackBuffer, bool inInheritParentClipping, bool inTransparent)
{
	...
	else
	{
		...
		if (((uLONG8)bounds.GetWidth())*((uLONG)bounds.GetHeight()) >= VGRAPHICCONTEXT_LAYER_SIZE_MAX)
		{
			//too big layer -> push empty layer
			SaveClip();
			return true;
		}
		...
		if (bmpRT == NULL)
		{
			//we prevent to fail on create target here as failing would screw up current rendering pass & force EndUsingContext to dispose & rebuild all resources
#if WITH_VMemoryMgr
			VSize freeMem = VMemory::GetManager()->GetFreeMemory();
			if (((VSize)bounds.GetWidth())*bounds.GetHeight()*4 > 2*freeMem/3)
			{
				//too big layer -> push empty layer
				SaveClip();
				return true;
		...
		if (FAILED(hr))
		{
			xbox_assert(false); //for D2D, it is fatal error: it will raise a recreate target error on EndDraw and EndUsingContext will dispose & rebuild all internal resources:
								//but we should never fall here as we check above free memory before
			if (bmpRT)
				bmpRT->Release();
			SaveClip();
			return true;
		...
		fStackLayerClip.push_back( VLayerClipElem( ctmOffset, bounds, fClipBoundsCur, fClipRegionCur, inBackBuffer, inAlpha, fHasClipOverride, ownBackBuffer, inFilterProcess));
		if (inInheritParentClipping)
		{
			fClipBoundsCur.Intersect( fStackLayerClip.back().GetBounds());
			if (fClipRegionCur)
...
void VWinD2DGraphicContext::BeginLayerShadow(const VRect& inBounds, const VPoint& inShadowOffset, GReal inShadowBlurDeviation, const VColor& inShadowColor, GReal inAlpha)
{
	...
	_BeginLayer( inBounds, inAlpha, filterProcess);
	ReleaseRefCountable(&filterProcess);
}
...
VImageOffScreenRef VWinD2DGraphicContext::_EndLayerOffScreen() const
{
	...
	VGraphicFilterProcess *filter = layer.GetFilter();
	if (filter && !fIsRTHardware) //TODO: supported only for software render target (it is 4D default)
	{
		//apply filter
		//(for now we apply filter using GDIPlus software implementation)

		D2D1_SIZE_U bmpSize = bmp->GetPixelSize();

		//first create GDIPlus bitmap from D2D bitmap (through a WIC bitmap render target)
		VWinD2DGraphicContext *gcWIC = new VWinD2DGraphicContext((sLONG)bmpSize.width, (sLONG)bmpSize.height, 1, VPoint(0,0), true, false);
		if (gcWIC)
		{
			xbox_assert(gcWIC);
			...
			rectf.left = 0;
			rectf.top = 0;
			rectf.right = bmpSize.width;
			rectf.bottom = bmpSize.height;
			rt->DrawBitmap( bmp, rectf, 1, D2D1_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR, rectf);
			gcWIC->EndUsingContext();
			IWICBitmap *bmpWIC = gcWIC->RetainWICBitmap();
			xbox_assert(bmpWIC);
			Gdiplus::Bitmap *bmpGDI = VPictureCodec_WIC::FromWIC(static_cast<IWICBitmapSource *>(bmpWIC));
			bmpWIC->Release();
			gcWIC->Release();

			if (bmpGDI)
			{
				//now apply filter on GDIPlus bitmap & get the resulting image (we do not draw yet to the gc)
				VGraphicImage *image = new VGraphicImage((VImageNativeRef)bmpGDI);
				delete bmpGDI;
				VGraphicImage* result = NULL;
				VRect boundsDest(0,0,bmpSize.width,bmpSize.height);
				filter->Apply( NULL, image, boundsDest, boundsDest, &result, eGraphicImageBlendType_Over, ctm.GetScaling());
				image->Release();

				if (result)
				{
					//finally create D2D bitmap from the result
					bmpGDI = (Gdiplus::Bitmap *)result->CreateNative();
					result->Release();
					bmp->Release();
					bmp = VPictureCodec_WIC::DecodeD2D( fRenderTarget, bmpGDI);
					delete bmpGDI;
				}
			}
		}
