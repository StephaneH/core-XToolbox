...
VectorOfSLONG ITextLayout::sTempVectorOfSLONG;
...
*/
VDocText* VTextLayout::CreateDocument(sLONG inStart, sLONG inEnd) const
{
	xbox_assert(false);
	return NULL;
}
...
VDocParagraph *VTextLayout::CreateParagraph(sLONG inStart, sLONG inEnd) const
{
	xbox_assert(false);
	return NULL;
}
...
void VTextLayout::GetMargins( sRectCSS& outMargins) const
{
...
/** get margins (in TWIPS) */
void VTextLayout::SetMargins( const sRectCSS& inMargins, bool /*inCalcAllMargin*/) //inCalcAllMargin is managed only by new layout
{
...
void VTextLayout::SetText(	const VString& inText, const VTreeTextStyle *inStyles, sLONG inStart, sLONG inEnd, 
							...
							const ISpanTextRef * inPreserveSpanTextRef, 
...
*/
bool VTextLayout::ReplaceAndOwnSpanRef( ISpanTextRef* inSpanRef, sLONG inStart, sLONG inEnd, bool inAutoAdjustRangeWithSpanRef, bool inSkipCheckRange, bool inNoUpdateRef,
...
bool  VTextLayout::_ApplyStyle(const VTextStyle *inStyle, bool inIgnoreIfEmpty)
{
	...
	if (inStyle->IsSpanRef())
	{
		ISpanTextRef *spanRef = inStyle->GetSpanRef();
...
#if VERSIONWIN
			if (start > 0 && inText[start-1] != 13 && inText[start-1] != 10 && inText[end-1] != 13 && inText[end-1] != 10)
			{
				//on Windows, include previous word or punctuation
				sLONG startWord = start, endWord = start;
				GetWordOrParagraphRange( inIntlMgr, inText, start-1, startWord, endWord, false, true, false);
				if (startWord < start)
					start = startWord;
			}
#endif
