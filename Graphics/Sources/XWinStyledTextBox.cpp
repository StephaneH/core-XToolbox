...
static bool sUseRiched20ForPrinting = true;
...
void XWinStyledTextBox::GetSizeForPrinting(GReal &ioWidth, GReal &outHeight)
{
	if (fTextServices)
	{
		...
		if (VSystem::IsEightOne() && sUseRiched20ForPrinting)
		{
...
bool XWinStyledTextBox::CharFormatFromVTextStyle(CHARFORMAT2W* ioCharFormat, const VTextStyle* inStyle, HDC inHDC, bool inParentHasBackColor)
{
	...
	if(fontname.GetLength() > 0)
	{
		...
		if (VFont::HasFontFamilySubstitute(fontname))
		{
			if (!VFont::FontGDIExists( fontname))
			{
				//try to substitute font family name
				VString newfontname;
				VFont::GetFontFamilySubstitute(fontname, newfontname, eWinGDIGraphicContext);
...
HRESULT XWinStyledTextBox::CreateTextServicesObject()
{
	...
	{
		if (sUseRiched20ForPrinting)
		{
			if (!sIsRiched20Loaded)
			{
				DWORD len, l;
				len = GetFileVersionInfoSizeW(L"msftedit.dll", &l);
				if (len)
				{
					LPVOID				p;
					p = GlobalAlloc(GMEM_FIXED, len);
					if (p)
					{
						VS_FIXEDFILEINFO	*vs;
						UINT				vslen;
						// version that fix the printing bug is 6.3.9600.17041
						if (GetFileVersionInfoW(L"msftedit.dll", 0, len, p))
						{
							if (VerQueryValueW(p, L"\\", (LPVOID*) &vs, &vslen))
							{
								long  vers = HIWORD(vs->dwFileVersionMS);
								long  maj = LOWORD(vs->dwFileVersionMS);
								long min = HIWORD(vs->dwFileVersionLS);
								long build = LOWORD(vs->dwFileVersionLS);

								if (vs->dwFileVersionMS == 0x00060003)// 6.3
								{
									if (vs->dwFileVersionLS >= 0x25804291) // 9600.17041
										sUseRiched20ForPrinting = false;
								}
								else if (vs->dwFileVersionMS > 0x00060003)
									sUseRiched20ForPrinting = false;
							}
						}
						GlobalFree(p);
