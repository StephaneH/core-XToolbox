...
bool XWinFontMgr::FontGDIExists(const VString& inName)
{
	VTaskLock lock(&fCriticalSection);

	if (std::find(fFontNames.begin(), fFontNames.end(), inName) != fFontNames.end())
		return true;

	//fixed ACI0086730: try to convert from postcript name to family name
	VString name(inName);
	name.ExchangeAll('-',' ');
	if (std::find(fFontNames.begin(), fFontNames.end(), name) != fFontNames.end())
		return true;
	return false;
}
