...
virtual						~XMacFontMgr ();
		...
		void				GetFontsSubstitutesFileName(VString& outFileName)
		{
			outFileName = "MacFontsSubstitutes.json";
		}

        /* convert localized font name to neutral */
        void                FontNameLocalizedToNeutral( VString& ioName) const;
    
static	CTFontUIFontType	StdFontToThemeFontID (StdFont inFont);
	
private:
         void                _UpdateMapOfFontNeutralNamePerLocalizedName( CTFontDescriptorRef inDesc);
    
mutable VCriticalSection	fCriticalSection;

typedef std::map<VString,VString> MapOfFontNeutralNamePerLocalizedName;

        /* reverse map for neutral font names (in order to get neutral font name from localized font name)
        @remarks
            map contains pairs for any kind of font name (family, display, ...)
        */
        MapOfFontNeutralNamePerLocalizedName    fFontNeutralNamePerLocalizedNames;
