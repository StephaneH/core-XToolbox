...
XBOX::VJSObject VJSEventEmitterClass::_ModuleConstructor (const VJSContext &inContext, const VString &inModuleName)
{
	...
	module = _NewInstance(inContext);
	module.SetProperty("EventEmitter", module, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete);
...
void VJSEventEmitterClass::_CallAsFunction (VJSParms_callAsFunction &ioParms)
{
	ioParms.ReturnValue(_NewInstance(ioParms.GetContext()));
}
...
XBOX::VJSObject VJSEventEmitterClass::_NewInstance (const XBOX::VJSContext &inContext)
{
	...
	if ((eventEmitter = new VJSEventEmitter()) == NULL) {
		...
		constructedObject = VJSEventEmitterClass::CreateInstance(inContext, eventEmitter);
