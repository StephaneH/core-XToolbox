...
{
public:
	VJSConstructor( JS4D::ClassDefinition* inClassRef, VJSParms_construct::fn_construct inCallback, bool inAcceptCallAsFunction)
...
VJSObject VJSConstructor::MakeConstructor( const VJSContext& inContext, JS4D::ClassRef inClassRef, VJSParms_construct::fn_construct inCallback, bool inAcceptCallAsFunction,JS4D::StaticFunction inContructorFunctions[] )
{
	...
	VJSConstructor*	newConstructor = new VJSConstructor(classDef,inCallback,inAcceptCallAsFunction);
...
void VJSConstructor::Callback(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	{
		v8::External*	extData = v8::External::Cast(*valData);
	...
	else
	{
		if (testAssert(vjsConstructor->fAcceptCallAsFunction))
		{
			...
			V4DContext*	v8Ctx = (V4DContext*)(args.GetIsolate()->GetData(0));

			v8::Persistent<v8::FunctionTemplate>*	pFT = v8Ctx->GetImplicitFunctionConstructor(vjsConstructor->fClassRef->className);
			v8::Local<v8::FunctionTemplate> locFT = Handle<FunctionTemplate>::New(args.GetIsolate(), *pFT);
		...
		else
		{
			...
			DebugMsg(tmpStr + "   !!!!\n");
			args.GetIsolate()->ThrowException(v8::String::NewFromTwoByte(args.GetIsolate(), tmpStr.GetCPointer(), v8::String::kNormalString, tmpStr.GetLength()));
			return;
		}
	}
}
