...
public:
	...
	static JS4D::ValueRef __cdecl js_callAsFunction( JS4D::ContextRef inContext, JS4D::ObjectRef inFunction, JS4D::ObjectRef inThis, size_t inArgumentCount, const JS4D::ValueRef inArguments[], JS4D::ExceptionRef* outException)
	{
		...
		fn( parms);
		JS4D::ValueRef tmpRef = parms.GetReturnValue().GetValueRef();
		return (tmpRef != NULL) ? tmpRef : JS4D::MakeNull(inContext).GetValueRef();
	}
#endif
		...
		*/
			void				StringifyWithSpaces(const VJSValue& inValue, sLONG inSpaces, XBOX::VString& outJSON, XBOX::VJSException *outException = NULL);
		
private:

#if USE_V8_ENGINE
			void				_Stringify(const XBOX::VJSValue& inValue, const XBOX::VJSValue inReplacer, const XBOX::VJSValue inSpace, XBOX::VString& outJSON, XBOX::VJSException *outException);
