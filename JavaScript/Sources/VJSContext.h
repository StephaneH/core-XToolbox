...
public:
#if USE_V8_ENGINE
	explicit					VJSException();
	explicit					VJSException(const VJSException& inOther);
	VJSException&				operator=(const VJSException& inOther);
								~VJSException();

	void						Clear();
	bool						IsEmpty() const;
	JS4D::ValueRef				GetValue() const;
	void						SetValue(JS4D::ContextRef inContext, const v8::Handle<v8::Value>& inExceptionValue);
	bool						ConvertErrorContextToException(JS4D::ContextRef inContext, VErrorContext *inErrorContext) { return JS4D::ConvertErrorContextToException(inContext, inErrorContext, this); }

#else
	explicit					VJSException() : fException(NULL)	{}
	explicit					VJSException(const VJSException& inOther) : fException(inOther.fException)	{}
	VJSException&				operator=(const VJSException& inOther) { fException = inOther.fException; return *this; }
								~VJSException() {}

	void						Clear() { fException = NULL; }
	bool						IsEmpty() const { return fException == NULL; }
	JS4D::ExceptionRef			GetValue() const { return fException; }
	bool						ConvertErrorContextToException(JS4D::ContextRef inContext, VErrorContext *inErrorContext) { return JS4D::ConvertErrorContextToException(inContext, inErrorContext, &fException); }
#endif
...
{
	return (XBOX::JS4D::ValueRef*)(inException ? &inException->fException : NULL);
}
#endif
private:
...
public:
	...
	explicit								VJSContext( JS4D::ContextRef inContext):fContext( inContext),fGlobalContext(NULL),fDebuggerAllowed(true)		{}
			...
			bool							EvaluateScript(const VString& inScript, const VURL *inSource, VJSValue *outResult, VJSException* outException, VJSObject* inThisObject = NULL, bool inCacheScript = true) const;
			...
			VJSObject						MakeFunction( const VString& inName, const VectorOfVString *inParamNames, const VString& inBody, const VURL *inUrl, sLONG inStartingLineNumber, VJSException *outException);
			bool							GetFunction(const VString& inFunctionRef, VJSObject& outFuncObj);
	
			VJSObject						GetGlobalObject() const;
#if USE_V8_ENGINE
			xbox::VString					GetCurrentScriptUrl() const;
#endif
...
public:
	...
	static	VJSGlobalContext*				Create( IJSRuntimeDelegate *inRuntimeDelegate);
			...
			bool							EvaluateScript(const VString& inScript, const VURL *inSource, VValueSingle **outResult, bool inJSONresult = false, bool inCacheScript = true) const;
...
private:
	explicit								VJSGlobalContext(JS4D::GlobalContextRef inGlobalContext);
