...
bool DetectUTF8( unsigned char* inContent, sLONG inContentSize )
{
	const unsigned char * bytes = (const unsigned char *) inContent;
	while ( *bytes && ( bytes + 4 - inContent < inContentSize ) )
	{
		if ( // ASCII
...
VError VStream::GuessCharSetFromLeadingBytes( CharSet inDefaultCharSet, bool inTryToDetectUTF8FromContent )
{
	...
	if ( (err == VE_OK) || (err == VE_STREAM_EOF) )
	{
		...
		else
		{
			...
			if ( inTryToDetectUTF8FromContent )
			{
				uBYTE buffer[ 2048 ];
				VSize bufferSize = Min<VSize>( 2048, GetSize() - GetPos() );
				GetData( buffer, &bufferSize );

				if ( DetectUTF8( buffer, bufferSize ) )
					charSet = VTC_UTF_8;

				UngetData( buffer, bufferSize );
			}
		}
	}
