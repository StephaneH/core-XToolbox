	...
	*/
	...
	static	sLONG				FindRawString( const UniChar* inText, sLONG inTextSize, const UniChar* inPattern, sLONG inPatternSize);
			...
			**/
			...
			VString&			AppendCString( const char* inCString);
	...
	virtual	bool				FromXMLString( const VString& inString);
			...
			void				FromUniString( const UniChar *inString, VIndex inLength);
			void				FromUniCString( const UniChar *inString)					{ FromUniString(inString, UniCStringLength(inString)); }

			// ToCString and FromCString are slow and uses a char encoding defined by the development environment.
			...
			void				FromCString( const char *inCString);
			...
			NSString*			MAC_RetainNSString() const									{ return (NSString*) MAC_RetainCFStringCopy();}
	...
	virtual	VError					GetJSONValue(VJSONValue& outJSONValue, bool allowDates = false) const;
	...
	virtual	VString*				Clone() const;
			...
			VString&			operator =( const char* inCString)				{ FromCString( inCString); return *this; }
