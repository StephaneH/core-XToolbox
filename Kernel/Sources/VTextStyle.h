...
typedef struct sRectCSS
{
	...
	bool operator ==(const sRectCSS& inRect) const { return left == inRect.left && top == inRect.top && right == inRect.right && bottom == inRect.bottom; }
} sRectCSS;
...
{
	kBACKGROUND_REPEAT = 0,
	kBACKGROUND_REPEAT_X,
	kBACKGROUND_REPEAT_Y,
	kBACKGROUND_NO_REPEAT
...
{
	kBACKGROUND_BORDER_BOX = 0,
	kBACKGROUND_PADDING_BOX,
	kBACKGROUND_CONTENT_BOX
...
auto				kBACKGROUND_SIZE_AUTO (no height)
cover				kBACKGROUND_SIZE_COVER (no height)
contain				kBACKGROUND_SIZE_CONTAIN (no height)
*/
typedef enum eBackgroundSize
{
	kBACKGROUND_SIZE_AUTO = 0,
	kBACKGROUND_SIZE_COVER = -10000,
	kBACKGROUND_SIZE_CONTAIN = -10001,
	kBACKGROUND_SIZE_UNDEFINED = -20000	//used to filter undefined width or height
...
{
	kLIST_STYLE_TYPE_FIRST = 0,

	kLIST_STYLE_TYPE_NONE = 0,

	kLIST_STYLE_TYPE_UL_FIRST,

	kLIST_STYLE_TYPE_DISC = kLIST_STYLE_TYPE_UL_FIRST,
	kLIST_STYLE_TYPE_CIRCLE,
	kLIST_STYLE_TYPE_SQUARE,

	kLIST_STYLE_TYPE_HOLLOW_SQUARE,
	kLIST_STYLE_TYPE_DIAMOND,
	kLIST_STYLE_TYPE_CLUB,
	kLIST_STYLE_TYPE_CUSTOM,

	kLIST_STYLE_TYPE_UL_LAST = kLIST_STYLE_TYPE_CUSTOM,
	kLIST_STYLE_TYPE_UL_4D_FIRST = kLIST_STYLE_TYPE_HOLLOW_SQUARE,
	kLIST_STYLE_TYPE_UL_4D_LAST = kLIST_STYLE_TYPE_CUSTOM,

	kLIST_STYLE_TYPE_OL_FIRST,

	kLIST_STYLE_TYPE_DECIMAL = kLIST_STYLE_TYPE_OL_FIRST,
	...
	kLIST_STYLE_TYPE_DECIMAL_GREEK,
	kLIST_STYLE_TYPE_OL_4D_FIRST = kLIST_STYLE_TYPE_DECIMAL_GREEK,
	kLIST_STYLE_TYPE_OL_4D_LAST = kLIST_STYLE_TYPE_DECIMAL_GREEK,

	kLIST_STYLE_TYPE_OL_LAST = kLIST_STYLE_TYPE_OL_4D_LAST,

	kLIST_STYLE_TYPE_LAST = kLIST_STYLE_TYPE_OL_LAST

} eListStyleType;

#define kLIST_START_AUTO	-1234567


/** span text reference type */
typedef enum eSpanTextRef
{
...
typedef enum eSpanRefTextType
{
	...
	kSRTT_Uniform_NBSP		= 8			//use NBSP character for uniform value but white space	
} eSpanRefTextType;
...
typedef enum eSpanRefCombinedTextType
{
	...
	kSRCTT_Uniform_NBSP			= 2048,			//use NBSP character for uniform value but white space
...
class XTOOLBOX_API ISpanTextRef
{
public:
	virtual							~ISpanTextRef() {}

	virtual	ISpanTextRef*			Clone() = 0;
	
	virtual bool					EqualTo(const ISpanTextRef *inSpanRef) const = 0;
	
	virtual	SpanRefTextTypeMask		CombinedTextTypeMaskToTextTypeMask(SpanRefCombinedTextTypeMask inCTT) = 0;

	virtual	eSpanTextRef			GetType() const = 0;
...
*/
class XTOOLBOX_API ISpanTextRefLayout
{
public:
	virtual	void					RetainRef() = 0;
	virtual	void					ReleaseRef() = 0;
protected:
	virtual							~ISpanTextRefLayout() {}
};

class XTOOLBOX_API ISpanTextRefFactory
{
public:
	virtual							~ISpanTextRefFactory()	{}
	static	ISpanTextRefFactory*	Get() { return fFactory; }
	static  void					Init(ISpanTextRefFactory* inFactory) { fFactory = inFactory; }
	static	void					DeInit() { if (fFactory) delete fFactory; fFactory = NULL; }

	virtual	ISpanTextRef*			Create(const eSpanTextRef inType, const VString& inRef, const VString& inDefaultValue = CVSTR(" ")) = 0;

private:
	static	ISpanTextRefFactory*	fFactory;
};
...
#if ENABLE_VTEXTSTYLE_EXT
class XTOOLBOX_API VTextStyle;
/** extended styles interface */
class XTOOLBOX_API ITextStyleExt
{
public:
	virtual							~ITextStyleExt() {}

	virtual	ITextStyleExt*			Clone() = 0;
	virtual bool					EqualTo(const ITextStyleExt *inStyle) const = 0;
	
	virtual void					InitFrom(const ITextStyleExt *inStyle) = 0;

	virtual bool					IsUndefined() const = 0;
	virtual void					Reset() = 0;
	virtual void					SetStyleToNormal() = 0;

	virtual	void					NotifyAttach(const VTextStyle *inParent) = 0;
	virtual	void					NotifyDetach() = 0;

			/** return mask of extended styles which are defined */
	virtual	uLONG					GetMaskSet() const = 0;

			/** merge current style with the specified style */
	virtual	void					MergeWithStyle( const ITextStyleExt *inStyle, const ITextStyleExt *inStyleParent = NULL, bool inForMetricsOnly = false) = 0;

			/** append styles from the passed style 
			@remarks
				if (inOnlyIfNotSet == true), overwrite only styles which are not defined locally
			*/
	virtual	void					AppendStyle( const ITextStyleExt *inStyle, bool inOnlyIfNotSet = false) = 0;

			/** reset styles which are equal to inStyle styles - inStyle must be not NULL */
	virtual	void					ResetCommonStyles( const ITextStyleExt *inStyle) = 0;

			/** reset styles which are not equal to inStyle styles - inStyle must be not NULL - and optionally set output mask to mask of styles which are not equal */
	virtual void					ResetUnCommonStyles( const ITextStyleExt *inStyle, uLONG* ioMaskMixed = NULL) = 0;

	virtual void					ClearUnderline() = 0;
	virtual void					ClearLineThrough() = 0;
};

class XTOOLBOX_API ITextStyleExtFactory
{
public:
	virtual ~ITextStyleExtFactory() {}
	static	ITextStyleExtFactory*	Get() { return fFactory; }
	static  void					Init(ITextStyleExtFactory* inFactory) { fFactory = inFactory; }
	static	void					DeInit() { if (fFactory) delete fFactory; fFactory = NULL; }

	virtual	ITextStyleExt*			Create() = 0;

private:
	static	ITextStyleExtFactory*	fFactory;
};
...
public:
	...
	typedef enum eStyle
	{
		...
		eStyle_Bold				=	1<<2,
		eStyle_Italic			=	1<<3,
		eStyle_Underline		=	1<<4,
		eStyle_Strikeout		=	1<<5,
		eStyle_Color			=	1<<6,
		eStyle_Just				=	1<<7,
		eStyle_BackColor		=	1<<8,
		eStyle_Ref				=	1<<9,

		eStyle_Max				=	eStyle_Ref,
		...
		eStyle_Mask				=	0xFFFFFFFF		//please do not modify it (important for proper ITextStyleExt impl)
		 
	} eStyle;

	VTextStyle(const VTextStyle* inTextStyle = NULL);
	VTextStyle(const VTextStyle& inTextStyle);
	...
	void		SetFontStyleToNormal(bool inShouldCreateStylesExt = false);
	...
	*/
	...
	void		Reset(bool inKeepRange = false);
	...
	*/
	...
	/** set and own span text reference (pass NULL to destroy current reference) */
	void		SetSpanRef( ISpanTextRef *inRef); 

	/** get span text reference  */
	ISpanTextRef *GetSpanRef() const { return fSpanTextRef; }

#if ENABLE_VTEXTSTYLE_EXT
	/** set and own extended styles reference (pass NULL to destroy current reference) */
	void		SetStylesExt( ITextStyleExt *inStylesExt); 

	/** get extended styles */
	ITextStyleExt *GetStylesExt() const { return fStylesExt; }

	/** get extended styles (create it if not done yet) */
	ITextStyleExt *GetStylesExtAndCreate();
#endif

protected:
	void	_Init(const VTextStyle *inStyle);
	...
	ISpanTextRef *fSpanTextRef;

#if ENABLE_VTEXTSTYLE_EXT
	ITextStyleExt *fStylesExt;
#endif
};
...
typedef sLONG (*fnGetStyleLong)(const VTextStyle *inStyle, sLONG& outUndefinedValue, uLONG inUserData);
	...
	*/
	static VIndex ReplaceStyledText(	VString& ioText, VTreeTextStyle*& ioStyles, const VString& inText, const VTreeTextStyle *inStyles = NULL, sLONG inStart = 0, sLONG inEnd = -1, 
									bool inAutoAdjustRangeWithSpanRef = true, bool inSkipCheckRange = false, bool inInheritUniformStyle = true, const ISpanTextRef * inPreserveSpanTextRef = NULL, void *inUserData = NULL, fnBeforeReplace inBeforeReplace = NULL, fnAfterReplace inAfterReplace = NULL);
	...
	@remarks
		it will insert plain text as returned by VSpanTextRef::GetDisplayValue() - depending so on span ref display type and on inNoUpdateRef (only one space plain text if inNoUpdateRef == true - if true, inNoUpdateRef changes span ref display type to uniform)
	...
	*/
	static bool ReplaceAndOwnSpanRef( VString& ioText, VTreeTextStyle*& ioStyles, ISpanTextRef* inSpanRef, sLONG inStart = 0, sLONG inEnd = -1, bool inAutoAdjustRangeWithSpanRef = true, bool inSkipCheckRange = false, bool inNoUpdateRef = true, void *inUserData = NULL, fnBeforeReplace inBeforeReplace = NULL, fnAfterReplace inAfterReplace = NULL, VDBLanguageContext *inLC = NULL);
	...
	*/
	...
	@remarks
		propagate flag to all VSpanTextRef & mark all as dirty
	*/ 
	void ShowSpanRefs( SpanRefCombinedTextTypeMask inShowSpanRefs = kSRCTT_Value);

	/** highlight or not span references 
	@remarks
		propagate flag to all VSpanTextRef 

		on default (if VSpanTextRef delegate is not overriden) it changes only highlight status for value
	...
	*/
	void ExpandAtPosBy(	sLONG inStart, sLONG inInflate = 1, const ISpanTextRef *inInflateSpanRef = NULL, VTextStyle **outStyleToPostApply = NULL, 
	...
	*/
	void BuildSequentialStylesFromCascadingStyles( std::vector<VTextStyle *>& outStyles, const VTextStyle *inStyleParent = NULL, bool inForMetrics = false, bool inFirstUniformStyleOnly = false) const;
	...
	*/
	void BuildSequentialStylesFromCascadingStyles( const VTextStyle *inStyleParent = NULL, bool inFirstUniformStyleOnly = false);
	...
	*/
	void BuildCascadingStylesFromSequentialStyles( std::vector<VTextStyle *>& inStyles, bool inKeepCurRange = false);
	...
	*/ 
	...
	/** this ensure decoration styles are not cascading (necessary for compatibility with most HTML browsers which do not manage well cascading decoration styles with span tags) 
	@remarks
		call this method for instance before exporting to HTML
		(method is recursive)
	*/
	void DisableCascadingForDecorations(const VTextStyle *inStyleInherit = NULL);
	...
	*/
	...
	/** get style on the specified range as sLONG value using a custom function
	@return
		true: style value is uniform on range
		false: style value is not uniform on range: return style value used by range start character
	*/
	bool		GetStyleAsLong( sLONG inStart, sLONG inEnd, sLONG& ioValue, fnGetStyleLong inFnGetStyle, uLONG inUserData = 0, VTextStyle *inStyleInherit = NULL) const;
...
public:
	VDocCache4DExp(VDBLanguageContext *inLC, const sLONG inMaxSize = kDOC_CACHE_4DEXP_SIZE) :VObject(), IRefCountable()
	{
		...
		fMaxSize = inMaxSize;
		fSpanRef = ISpanTextRefFactory::Get() ? ISpanTextRefFactory::Get()->Create(kSpanTextRef_4DExp, CVSTR("")) : NULL;
...
public:
	...
	*/
	...
	/** replace text with span text reference on the specified range
	@remarks
		it will insert plain text as returned by VSpanTextRef::GetDisplayValue() - depending so on local display type and on inNoUpdateRef (only NBSP plain text if inNoUpdateRef == true - inNoUpdateRef overrides local display type)
		...
		- if display type is set to normalized source)
	*/
	virtual	bool				IDOC_ReplaceAndOwnSpanRef(ISpanTextRef* inSpanRef, sLONG inStart = 0, sLONG inEnd = -1, bool inNoUpdateRef = false, VDBLanguageContext *inLC = NULL) = 0;
	...
	virtual sLONG				IDOC_GetReferenceAtRange(	const sLONG inStart = 0, const sLONG inEnd = -1, sLONG *outFirstRefStart = NULL, sLONG *outFirstRefEnd = NULL, VString *outFirstRefSource = NULL, VString *outFirstRefValue = NULL);
...
{
	kHTML_PARSER_TAG_IMAGE_SUPPORTED			=	1<<0,	//if enabled, parser will parse image tags as image span references otherwise as unknown tags 
	kHTML_PARSER_TAG_TABLE_SUPPORTED			=	1<<1,	//if enabled, parser will parse table tags as table span references otherwise as unknown tags //TODO: tables are not implemented yet
	
	kHTML_PARSER_DISABLE_XHTML4D				=	1<<3,	//if enabled, parser will ignore 4D namespace if present & parse XHTML4D as XHTML11, ignoring any 4D reserved style

	kHTML_PARSER_PRESERVE_WHITESPACES			=	1<<4,	//if enabled, parser will preserve whitespaces on default (for legacy XHTML, default is to consolidate whitespaces)
															//(useful only for legacy XHTML because for XHTML4D, paragraph whitespaces are preserved with 'white-space:pre-wrap') 

	kHTML_PARSER_SPAN_EXT_SUPPORTED				=	1<<5	//if enabled, parser will parse extended span styles (& also font size on Windows is assumed to be platform-independent)
...
typedef enum eHTMLSerializerOptions
{
	kHTML_SERIALIZER_WITH_DEFAULT_STYLES		= 1<<0,		//if enabled, serializer will serialize default CSS styles from passed inDocDefaultStyleMgr (styles associated with empty class name)
	
	kHTML_SERIALIZER_WITH_4DEXP_VALUES			= 1<<1,		//if enabled, serializer will serialize computed values from 4D expressions as plain text (if disabled - default, it serializes only one no-breaking space as 4D expression plain text)
		
	kHTML_SERIALIZER_XHTML4D_PRETTY_PRINT		= 1<<2,		//if enabled and if document version is xhtml4D, serializer will serialize also end of lines and indentation between tags (but inside paragraphs whom text content is preserved) for pretty formatting

	kHTML_SERIALIZER_SPAN_EXT_SUPPORTED			= 1<<3,		//if enabled, will serialize also extended span styles (and platform-independent font size on Windows) 

	kHTML_SERIALIZER_WITH_CIDS					= 1<<4,		//if enabled, will serialize embedded or local images as CID urls and return CID references & picture table (can be parsed again but such images urls if present in html cannot be displayed)

	kHTML_SERIALIZER_MIME_HTML					= 1<<5		//if enabled, will serialize document as MIME multi-part html (text is encoded as 7-bit characters)

} eHTMLSerializerOptions;

#define kHTML_SERIALIZER_OPTIONS_DEFAULT		0
...
{
public:
typedef std::vector<VPicture *> VectorOfPicturePtr;

virtual ~ISpanTextParser() {}
#if VERSIONWIN
...
*/
virtual void GenerateSpanText( const VTreeTextStyle& inStyles, const VString& inPlainText, VString& outTaggedText, VTextStyle* inDefaultStyle = NULL, bool inLegacy = true) = 0;
...
/** replace styled text with span text reference on the specified range
	...
	you should no more use or destroy passed inSpanRef even if method returns false
*/
virtual bool ReplaceAndOwnSpanRef( VString& ioSpanText, ISpanTextRef* inSpanRef, sLONG inStart = 0, sLONG inEnd = -1, bool inSilentlyTrapParsingErrors = true) = 0;
...
/** return the first span reference which intersects the passed range 
@return
	eSpanTextRef enum value or -1 if none ref found
*/
...
@param inDoc
	document to serialize: inDoc->GetVersion() defines the output html format (span4D, html4D, or html1.1)
@param outDocText
	serialized document 
@param inDocDefaultStyleMgr
	if not NULL, should contain default node style sheets (only one style sheet per node type with empty class name)
@param inOptions
	serialize options: see eHTMLSerializerOptions
@param outCIDUrls
	if option kHTML_SERIALIZER_WITH_CIDS is set, will contain CID urls (without "cid:") for CID-converted embedded or local images
@param outCIDPictures
	if option kHTML_SERIALIZER_WITH_CIDS is set, will contain CID pictures for CID-converted embedded or local images (vector size should be equal to outCIDUrls vector size)
	(caller is owner of pictures)
@param inCIDHostDomain
	if option kHTML_SERIALIZER_WITH_CIDS is set, inHostDomain will be added to auto-generated CID urls with a "@" separator (for instance "id100@gmail.com" if inHostDomain = "gmail.com")
*/
virtual void SerializeDocument(const VDocText *inDoc, VString& outDocText, const VDocClassStyleManager *inDocDefaultStyleMgr = NULL, uLONG inOptions = kHTML_SERIALIZER_OPTIONS_DEFAULT, VectorOfVString *outCIDUrls = NULL, VectorOfPicturePtr *outCIDPictures = NULL, const VString* inCIDHostDomain = NULL) = 0;
