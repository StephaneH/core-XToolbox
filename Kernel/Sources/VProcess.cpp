...
bool VProcess::_Init( VProcess::InitOptions inOptions)
{
	...
	if (ok && ( (inOptions & Init_EnableLogger) != 0) )
	{
...
#elif VERSIONMAC
	minimalSystemVersion = MAC_OSX_10_7;
#elif VERSION_LINUX
...
#elif VERSIONMAC
		VString msg = (inMinimalSystemVersion == MAC_OSX_10_9) ? "This application needs at least OS X 10.9" : "This application needs at least OS X 10.7";
#elif VERSION_LINUX
...
void VProcess::_ReadProductName( VString& outName) const
{
	...
	(void) outName; // avoid compiler warning
...
void VProcess::_ReadProductVersion( VString& outVersion) const
{
	...
	(void) outVersion; // avoid compiler warning
#endif
}
