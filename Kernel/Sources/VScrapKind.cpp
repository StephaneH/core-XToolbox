...
ScrapKind SCRAP_KIND_SPAN						= "com.4d.private.text.span";		//legacy span - on Windows, font size is design font size in point*72/screen dpi while it is design font size in point on Mac (v13 compatible) 
ScrapKind SCRAP_KIND_SPAN_EXT					= "com.4d.private.text.spanext";	//extended span - font size is platform-independent (design font size in point); has support also for extended styles like text-transform, text-shadow etc...
																					//(used only for now by Write object - in order to be rid off legacy span not platform-independent font size & support for extended styles)
