...
VError VDuration::GetJSONValue(VJSONValue& outJSONValue, bool allowDates) const
{
	if (IsNull())
...
VError VTime::GetJSONString(VString& outJSONString, JSONOption inModifier) const
{
	...
	{
		VStr255 temp;
		GetRfc3339String( temp, XSO_Default);
		outJSONString = '\"';
		outJSONString += temp;
		outJSONString += '\"';
	}
	...
	{
		GetRfc3339String( outJSONString, XSO_Default);
	}
	return VE_OK;
}
...
VError VTime::GetJSONValue(VJSONValue& outJSONValue, bool allowDates) const
{
	if (IsNull())
		outJSONValue.SetNull();
	else
