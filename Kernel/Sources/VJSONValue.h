...
{
	friend class VJSONBinaryWriter;
public:
...
{
	BJSON_AllowDates = 64,
	BJSON_Default = BJSON_AllowDates
};
typedef uLONG BJSONOption;

/*
	VJSONBinaryWriter let you stream to binary a VJSONValue, a VJSONObject or a VJSONArray.
	
	Options let you control the way things are streamed.
*/
class XTOOLBOX_API VJSONBinaryWriter : public VObject
{
public:
			// pass option JSON_WithQuotesIfNecessary for standard JavaScript stringification
	explicit						VJSONBinaryWriter(BJSONOption inOptions = BJSON_Default);

			BJSONOption				GetOptions() const					{ return fOptions;}
			
			// Recursively streams value, object or array to binary format.
			// Cyclic structures are detected and throw error VE_JSON_STRINGIFY_CIRCULAR.
			VError					StreamValue( const VJSONValue& inValue, VMemoryBuffer<>& outBuff);
			VError					StreamObject(const VJSONObject *inObject, VMemoryBuffer<>& outBuff);
			VError					StreamArray(const VJSONArray *inArray, VMemoryBuffer<>& outBuff);
			
private:
			BJSONOption				fOptions;
			std::vector<const VJSONObject*>	fStack;	// used to detect recursion
};
...
/*
VJSONBinaryImporter lets build a VJSONValue that was previously streamed to binary.

*/

class XTOOLBOX_API VJSONBinaryImporter : public VObject
{
	public:
		VJSONBinaryImporter(const void* from, VSize len)
		{
			fCurPtr = (uBYTE*)from;
			fLen = len;
		}

		VError GetValue(VJSONValue& outVal);
	private:
		uBYTE* fCurPtr;
		VSize fLen;
};
