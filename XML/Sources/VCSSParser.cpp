...
const VString kCSS_TEXT_DECORATION_STYLE_SOLID				= CVSTR("solid");
const VString kCSS_TEXT_DECORATION_STYLE_DOTTED				= CVSTR("dotted");
const VString kCSS_TEXT_DECORATION_STYLE_DASHED				= CVSTR("dashed");
const VString kCSS_TEXT_DECORATION_STYLE_DOUBLE				= CVSTR("double");
const VString kCSS_TEXT_DECORATION_STYLE_SEMI_TRANSPARENT	= CVSTR("semi-transparent");
const VString kCSS_TEXT_DECORATION_STYLE_WORD				= CVSTR("word");

const VString kCSS_TEXT_TRANSFORM_CAPITALIZE				= CVSTR("capitalize");
const VString kCSS_TEXT_TRANSFORM_LOWERCASE					= CVSTR("lowercase");
const VString kCSS_TEXT_TRANSFORM_UPPERCASE					= CVSTR("uppercase");
const VString kCSS_TEXT_TRANSFORM_SMALL_UPPERCASE			= CVSTR("small-uppercase");
...
CSSProperty::Value *VCSSParser::ParseValue( VCSSLexParser *inLexParser, CSSProperty::eType inType, VString* ioValue, bool inIgnoreSVG, bool inParseFirstTokenOnly)
{
	...
	if (!done)
		...
		//4D CSS reserved 
		case CSSProperty::TEXTDECORATIONSTYLE4D:
			while (inLexParser->GetCurToken() != CSSToken::END)
			{
				switch( inLexParser->GetCurToken())
				{
				case CSSToken::IDENT:
					...
					else
					{
						if (inLexParser->GetCurTokenValue().EqualToString( kCSS_INHERIT, false))
							value->fInherit = true;
						else if (inLexParser->GetCurTokenValue().EqualToString( kCSS_TEXT_DECORATION_STYLE_SOLID, false))
							value->v.css.fTextDecorationStyle = CSSProperty::kTEXT_DECORATION_STYLE_SOLID;
						else if (inLexParser->GetCurTokenValue().EqualToString( kCSS_TEXT_DECORATION_STYLE_DOTTED, false))
							value->v.css.fTextDecorationStyle = CSSProperty::kTEXT_DECORATION_STYLE_DOTTED;
						else if (inLexParser->GetCurTokenValue().EqualToString( kCSS_TEXT_DECORATION_STYLE_DASHED, false))
							value->v.css.fTextDecorationStyle = CSSProperty::kTEXT_DECORATION_STYLE_DASHED;
						else if (inLexParser->GetCurTokenValue().EqualToString( kCSS_TEXT_DECORATION_STYLE_DOUBLE, false))
							value->v.css.fTextDecorationStyle = CSSProperty::kTEXT_DECORATION_STYLE_DOUBLE;
						else if (inLexParser->GetCurTokenValue().EqualToString( kCSS_TEXT_DECORATION_STYLE_SEMI_TRANSPARENT, false))
							value->v.css.fTextDecorationStyle = CSSProperty::kTEXT_DECORATION_STYLE_SEMI_TRANSPARENT;
						else if (inLexParser->GetCurTokenValue().EqualToString( kCSS_TEXT_DECORATION_STYLE_WORD, false))
							value->v.css.fTextDecorationStyle = CSSProperty::kTEXT_DECORATION_STYLE_WORD;
						else
						{
							XBOX::ReleaseRefCountable(&value);
							return NULL;
		...
		case CSSProperty::TEXTTRANSFORM:
		case CSSProperty::TEXTTRANSFORM4D:
			while (inLexParser->GetCurToken() != CSSToken::END)
			{
				switch( inLexParser->GetCurToken())
				{
				case CSSToken::IDENT:
					...
					else
					{
						if (inLexParser->GetCurTokenValue().EqualToString( kCSS_INHERIT, false))
							...
							value->v.css.fTextTransform = CSSProperty::kTEXT_TRANSFORM_NONE;
						else if (inLexParser->GetCurTokenValue().EqualToString( kCSS_TEXT_TRANSFORM_CAPITALIZE, false))
							value->v.css.fTextTransform = CSSProperty::kTEXT_TRANSFORM_CAPITALIZE;
						else if (inLexParser->GetCurTokenValue().EqualToString( kCSS_TEXT_TRANSFORM_LOWERCASE, false))
							value->v.css.fTextTransform = CSSProperty::kTEXT_TRANSFORM_LOWERCASE;
						else if (inLexParser->GetCurTokenValue().EqualToString( kCSS_TEXT_TRANSFORM_UPPERCASE, false))
							value->v.css.fTextTransform = CSSProperty::kTEXT_TRANSFORM_UPPERCASE;
						else if (inLexParser->GetCurTokenValue().EqualToString( kCSS_TEXT_TRANSFORM_SMALL_UPPERCASE, false))
							value->v.css.fTextTransform = CSSProperty::kTEXT_TRANSFORM_SMALL_UPPERCASE;
						else
						{
							XBOX::ReleaseRefCountable(&value);
							return NULL;
