...
void VProcessLauncher::SetQuoteArgumentsIfNeeded(bool inQuoteArgumentsIfNeeded)
{
	fProcessLauncherImpl->SetQuoteArgumentsIfNeeded(inQuoteArgumentsIfNeeded);
}
...
void VProcessLauncher::EnableProcessTreeKill ()
{
	fProcessLauncherImpl->EnableProcessTreeKill();
}
...
sLONG VProcessLauncher::Shutdown(bool inWithKillIndependantChildProcess, bool inKillProcessTree)
{
	return fProcessLauncherImpl->Shutdown(inWithKillIndependantChildProcess, inKillProcessTree);
}
...
sLONG VProcessLauncher::ExecuteCommandLine(const VString& inCommandLine,
										   ...
										   bool inQuoteArgumentsIfNeeded,
										   bool inKillProcessTree)
{
	sLONG				err = -1;
...
*/
	...
	processLauncher-> SetQuoteArgumentsIfNeeded ( inQuoteArgumentsIfNeeded );
	...
	if (inKillProcessTree)
		...
		processLauncher->EnableProcessTreeKill();
...
#define BUFFER_SIZE	4096
		...
		if (outProcessLauncher == NULL) {
			...
			if (inKillProcessTree)
				...
				err = processLauncher->Shutdown(false, true); 
