...
{	
	...
	fQuoteArgumentsIfNeeded = true;
	
	fUseProcessGroup = false;
...
void XPosixProcessLauncher::SetQuoteArgumentsIfNeeded(bool inQuoteArgumentsIfNeeded)
{
	fQuoteArgumentsIfNeeded = inQuoteArgumentsIfNeeded;
}
	...
	// -------------------------------------------------
		{
			...
			// Make child a process group (its pid will be the pgid). All its children will inherit this group id and it can then be possible to kill them all.
			
			error = setpgid(getpid(), 0);
			if (error) {
				
				fprintf(stderr, "********** XPosixProcessLauncher::Start/fork() -> child -> Error: setpgid failed (%s)\n", strerror(errno));
				fflush(stderr);
				_Exit(-1);
...
sLONG XPosixProcessLauncher::Shutdown(bool inWithKillIndependantChildProcess, bool inKillProcessTree)
{
	int error = 0;
	int status = 0;
	...
	if(IsRunning())
	{
		...
		else if(inWithKillIndependantChildProcess)
		{
			...
			if(IsRunning()) {
				...
				if(!_TerminateIndependantChildProcess(SIGTERM, 5000, false))
				{
					if(!_TerminateIndependantChildProcess(SIGQUIT, 5000, false))
					{
						if(!_TerminateIndependantChildProcess(SIGKILL, 5000, false))
						{
	...
	if (inKillProcessTree) {
		
		xbox_assert(fUseProcessGroup);
		if (fUseProcessGroup)
			
			kill(fProcessGroupID, SIGKILL);
...
bool XPosixProcessLauncher::_TerminateIndependantChildProcess(int inSignal, sLONG inTimeOutInMS, bool inKillProcessTree)
{
	xbox_assert(!(inKillProcessTree && !fUseProcessGroup));
	...
	// For a process group ID kill, Mac would prefer killpg() instead of kill() with negative pid (process group ID).
	// But this is accepted as compatibility, so that's ok.
		
	int	processID;
	
	processID = inKillProcessTree ? fProcessGroupID : fProcessID;
	if(kill(processID, inSignal) == 0)
	{
