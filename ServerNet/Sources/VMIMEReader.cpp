...
bool VMIMEReader::_GuessBoundary()
{
	...
	if (VE_OK == err)
	{
		...
		if (HTTPTools::BeginsWithASCIICString (lineString, "--"))
		{
			lineString.GetSubString(3, lineString.GetLength() - 2, fBoundary); // YT 02-Jul-2014 - WAK0087525
		}
		else
...
XBOX::VError VMemoryBufferStream::DoSetPos (sLONG8 inNewPos)
{
	...
	if (inNewPos >= (sLONG8) fStartIndexes[fCurrentMemoryBufferIndex]) {

		// Most common case is a DoSetPos() inside one memory buffer, without "crossing".

		if (inNewPos < (sLONG8) fStartIndexes[fCurrentMemoryBufferIndex + 1])
			...
			for ( ; ; )
				...
				} else if (inNewPos < (sLONG8) fStartIndexes[index]) {
		...
		for ( ; ; )
			...
			if (!index) {
				...
				index--;
					if (inNewPos >= (sLONG) fStartIndexes[index]) {
...
void VMemoryBufferStream::_initialize (const XBOX::VMemoryBuffer<> *inMemoryBuffers, sLONG inNumberMemoryBuffers)
{
	...
	if ((fStartIndexes = (VSize *) XBOX::vMalloc(sizeof(VSize) * (fNumberMemoryBuffers + 1), 0)) == NULL)
		...
		j = 0;
		for (i = 0; i < (VSize) fNumberMemoryBuffers; i++) {
