...
namespace SslFramework
{
    ...
    //Manipulate ciphers suite. Default means changes are applied on global context,
    //and propagate to new connexions contexts.
    XTOOLBOX_API VError SetDefaultCipherList(const VString inCipherList=CVSTR(""));
    XTOOLBOX_API VError GetDefaultCipherList(VString* ioCipherList);

    //VKeyCertChain is a ServerNet opaque type to deal with keys and certificates.
    //You can create it, retain it or release it (because it may live outside of ServerNet), but you can not manipulate it.
	XTOOLBOX_API VKeyCertChain* RetainKeyCertChain(const VMemoryBuffer<>& inKeyBuffer, const VMemoryBuffer<>& inCertBuffer);
	XTOOLBOX_API VKeyCertChain* RetainKeyCertChain(const VFolder& inCertFolder,
                                                   const VString& inKey=CVSTR("key.pem"), const VString& inCert=CVSTR("cert.pem"));
	XTOOLBOX_API VKeyCertChain* RetainKeyCertChain(VKeyCertChain* inKeyCertChain);
	XTOOLBOX_API void ReleaseKeyCertChain(VKeyCertChain** inKeyCertChain);
	XTOOLBOX_API VError LoadIntoGlobalContext(VKeyCertChain* inKeyCertChain);
