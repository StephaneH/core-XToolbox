...
VServerNetManager::VServerNetManager() :
	fCriticalError(NULL), fClientCertificateSource(NULL),
	fDefaultClientIdleTimeOut(20000 /*20s*/), fSelectIOInactivityTimeOut(300 /*ms*/),
	fEndPointCounter(0), fIpPolicy(IpForceV4), fIpStacks(0), fInitCount(0)
...
VKeyCertChain* VServerNetManager::RetainClientCertificate()
{
	if(fClientCertificateSource!=NULL)
		return fClientCertificateSource->RetainKeyCertChain();

	return NULL;
}


void VServerNetManager::SetClientCertificateSource(IKeyCertChainSource* inKeyCertChainSrc)
{
    fClientCertificateSource=inKeyCertChainSrc;
}
...
VKeyCertChain* ServerNetTools::RetainClientKeyCertChain()
{
	return VServerNetManager::Get()->RetainClientCertificate();
}
...
void ServerNetTools::Set_4D_DefaultSSLPrivateKey(const VMemoryBuffer<>& inKeyBuffer)
{
	VError verr=SslFramework::SetDefaultPrivateKey(inKeyBuffer);

	xbox_assert(verr==VE_OK);
}


void ServerNetTools::Set_4D_DefaultSSLCertificate(const VMemoryBuffer<>& inCertBuffer)
{
	VError verr=SslFramework::SetDefaultCertificate(inCertBuffer);
...
VError ServerNetTools::GetResourcesCertificateAndKey(VMemoryBuffer<>& outCertBuffer, VMemoryBuffer<>& outKeyBuffer)
{
    VError verr=VE_FILE_NOT_FOUND;
    
    VFolder* appFolder=VProcess::Get()->RetainFolder(VProcess::eFS_Resources);
    
    if(appFolder!=NULL)
    {
        VFile* certFile=appFolder->RetainRelativeFile(CVSTR("cert.pem"), FPS_POSIX);
        
        if(certFile!=NULL)
        {
            verr=certFile->GetContent(outCertBuffer);
      
            ReleaseRefCountable(&certFile);
            
            if(verr==VE_OK)
            {
                VFile* keyFile=appFolder->RetainRelativeFile(CVSTR("key.pem"), FPS_POSIX);
                
                if(keyFile!=NULL)
                {
                    verr=keyFile->GetContent(outKeyBuffer);
        ...
        ReleaseRefCountable(&appFolder);
    }
    
    //Default cert.pem and key.pem should always exist in resources
    xbox_assert(verr==VE_OK);
...
VError ServerNetTools::GetResourcesCertificateAndKey(VString& outCertString, VString& outKeyString)
{
    VMemoryBuffer<> certBuffer, keyBuffer;
    
    VError verr=GetResourcesCertificateAndKey(certBuffer, keyBuffer);
    
    if(verr==VE_OK)
    {
        outCertString.FromBlock(certBuffer.GetDataPtr(), certBuffer.GetDataSize(), XBOX::VTC_UTF_8);

        outKeyString.FromBlock(keyBuffer.GetDataPtr(), keyBuffer.GetDataSize(), XBOX::VTC_UTF_8);
    }
