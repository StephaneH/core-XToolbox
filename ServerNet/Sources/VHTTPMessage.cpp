...
char *_FindEndOfLine(char *inDataBuffer, sLONG& outEndOfLineSize)
{
	char *endLinePtr = strstr(inDataBuffer, "\r\n");

	if (NULL == endLinePtr)
	{
		endLinePtr = strstr(inDataBuffer, "\n");

		if (NULL == endLinePtr)
			endLinePtr = strstr(inDataBuffer, "\r");
...
char *_FindEndOfHeader(char *inDataBuffer, sLONG& outEndOfHeaderSize)
{
	char *endHeaderPtr = strstr(inDataBuffer, "\r\n\r\n");

	if (NULL == endHeaderPtr)
	{
		endHeaderPtr = strstr(inDataBuffer, "\n\n");

		if (NULL == endHeaderPtr)
			endHeaderPtr = strstr(inDataBuffer, "\r\r");

		if (NULL != endHeaderPtr)
			outEndOfHeaderSize = 2;
		else
			outEndOfHeaderSize = 0;
	}
	else
	{
		outEndOfHeaderSize = 4;
	}
	...
	if ((NULL != bufferPtr) && (bufferSize > 0))
	{
		...
		while ((bufferPtr < bufferEndPtr) && !bStopReadingStream)
		{
			if (parsingState <= PS_ReadingHeaders)
			{
				...
				endLinePtr = _FindEndOfLine(startLinePtr, endLineSize);

				if ((NULL != endLinePtr) && (NULL == endHeaderPtr))
					endHeaderPtr = _FindEndOfHeader(startLinePtr, endHeaderSize);
			}
				...
				case PS_ReadingBody:
				{
					if (NULL != boundary)
					{
						...
						if (NULL != endBoundaryPtr)
						{
							...
							if (NULL != memstr(endBoundaryPtr - 2, 2, "\r\n", 2))
								unreadBytes -= 2;	// Skip CRLF after boundary part
							else if ((NULL != memstr(endBoundaryPtr - 1, 1, "\r", 1)) || (NULL != memstr(endBoundaryPtr - 1, 1, "\n", 1)))
								unreadBytes -= 1;	// Skip CR or LF after boundary part
